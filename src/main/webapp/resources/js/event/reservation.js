...
for (const palaceName of palaceNames) {
    palaceName.addEventListener('click', () => {
        ...
        console.log('Royal ID:', royal_id);
         ...
         let event_id;
         switch (royal_id) {
             case '1': event_id = 40; break; // 경복궁
             case '4': event_id = 41; break; // 덕수궁
             case '3': event_id = 42; break; // 창경궁
             case '2': event_id = 43; break; // 창덕궁
             case '5': event_id = 44; break; // 종묘
             default: break;
         }
                ...
                $.ajax({
                    ...
                    success: (data)=>{
                        ...
                        eventData.forEach((event) => {
                            ...
                            reservationInfo.textContent = `예약 인원: ${event.curr_resv_num}/${event.round_capacity}`;
